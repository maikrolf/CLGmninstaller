...
case $key in
    ...
    -h|--help)
    cat << EOL
clg Masternode installer arguments:
...
# Set these to change the version of collegicoin to install
TARBALLURL="https://github.com/CollegicoinCLG/collegicoin/releases/download/v4.0.3/collegicoin-v4.0.3-linux.tar.gz"
TARBALLNAME="collegicoin-v4.0.3-linux.tar.gz"
BOOTSTRAPURL=""
BOOTSTRAPARCHIVE=""
BWKVERSION="1.0.0"
...
echo "
 ...
 | the CLG Team CANNOT assist you in repairing  |::
...
echo "" && echo 'Added user "collegicoin"' && echo ""
sleep 1
...
rm $TARBALLNAME
mv ./collegicoind /usr/local/bin
mv ./collegicoin-cli /usr/local/bin
mv ./collegicoin-tx /usr/local/bin
rm -rf $TARBALLNAME

# Create collegicoincore directory
mkdir /root/.collegicoincore
...
touch /root/.collegicoincore/collegicoin.conf
cat > /root/.collegicoincore/collegicoin.conf << EOL
${INSTALLERUSED}
rpcuser=${RPCUSER}
...
masternode=1
EOL
chmod 0600 /root/.collegicoincore/collegicoin.conf
chown -R $USER:$USER /root/.collegicoincore

sleep 1

cat > /etc/systemd/system/collegicoin.service << EOL
[Unit]
...
ExecStart=/usr/local/bin/collegicoind -conf=/root/.collegicoincore/collegicoin.conf -datadir=/root/.collegicoincore
ExecStop=/usr/local/bin/collegicoin-cli -conf=/root/.collegicoincore/collegicoin.conf -datadir=/root/.collegicoincore stop
Restart=on-abort
[Install]
WantedBy=multi-user.target
EOL
sudo systemctl enable collegicoin.service
sudo systemctl start collegicoin.service
